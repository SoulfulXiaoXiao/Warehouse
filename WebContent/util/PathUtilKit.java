...
public class PathUtilKit {
	...
	public static String getRootClassPath() {
		if (rootClassPath == null) {
			try {
				String path = PathUtilKit.class.getClassLoader().getResource("").toURI().getPath();
				rootClassPath = new File(path).getAbsolutePath();
			}
			catch (Exception e) {
				String path = PathUtilKit.class.getClassLoader().getResource("").getPath();
				rootClassPath = new File(path).getAbsolutePath();
			}
		}
	...
	public void setRootClassPath(String rootClassPath) {
		PathUtilKit.rootClassPath = rootClassPath;
	}
	...
	public static void setWebRootPath(String webRootPath) {
		...
		PathUtilKit.webRootPath = webRootPath;
	}
	
	// 注意：命令行返回的是命令行所在路径的上层的上层路经
	private static String detectWebRootPath() {
		try {
			String path = PathUtilKit.class.getResource("/").toURI().getPath();
